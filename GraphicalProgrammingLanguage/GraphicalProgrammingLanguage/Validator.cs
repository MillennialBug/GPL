...
namespace GraphicalProgrammingLanguage
{
    internal class Validator
    {
        ...
        private List<String> shapes = new List<String>() { "circle", "star" };

        public void validateCommand(String[] cmd)
        {
            if (!validCommands.ContainsKey(cmd[0]))
            {
                throw new Exception("Bad command found: " + cmd[0]);
            }
            if(!(cmd.GetLength(0) > 1))
            {
                throw new Exception("No arguments provided: " + cmd.ToString());
            }
        ...
        private void validateArgs(String cmd, String[] args)
        {
            ...
            if (validCommands.TryGetValue(cmd, out pattern))
            {
                ...
                {
                    throw new Exception("Bad arguments found: " + args[0]);
                }
            }
        }
